...
export default function SidebarTree({ uuid, level = 0, isPublic }: Props) {
  const params = useParams();
  const paramuuid = params.pages && params.pages[0];
  ...
  const {
    ...
    publicSidebarTree,
    privateSidebarTree,
    getPersonalSidebarTreeAsync,
    getPrivateSidebarTreeAsync,
  ...
  useEffect(() => {
    if (isPublic) {
      getPrivateSidebarTreeAsync(uuid);
    } else {
      getPersonalSidebarTreeAsync(uuid);
    }
  }, [isPublic, uuid, getPersonalSidebarTreeAsync, getPrivateSidebarTreeAsync]);

  // first loading at root level
  if (loading["root"] && !publicSidebarTree && !privateSidebarTree && !uuid) {
    return <SidebarTree.Skeleton level={level} />;
  }

  const navigateDocHandler = (uuid: string) => {
    triggerMinimize("documents");
    if (paramuuid === uuid) return;
    router.push(`/documents/${uuid}`);
  };

  const sidebarTree = isPublic ? privateSidebarTree : publicSidebarTree;
  const items = getSidebarTreeData(sidebarTree, uuid);

  if (loading[uuid!] && !items.length)
    return <SidebarTree.Skeleton level={level} />;
  if (!items.length) return <SidebarTree.Empty level={level} />;
  return (
    <>
      {/* <SidebarTree.Title level={level} isPublic={isPublic} /> */}
              ...
              <div className="flex items-center justify-start truncate">
                ...
                <span
                  className={cn(
                    ...
                    paramuuid === item.uuid && "font-semibold text-primary",
                  )}
                >
                  {item.title}
                </span>
...
type LevelProps = { level: number; isPublic?: boolean };

SidebarTree.Title = function Title({ level, isPublic }: LevelProps) {
  if (level > 0) return null;
  return (
    <h2 className="mb-1 px-3 pt-3 text-xs text-muted-foreground">
...
SidebarTree.Skeleton = function Loading({ level }: LevelProps) {
  return (
    <div className={cn(level === 0 ? "pt-3" : "pt-1")}>
      {level === 0 && (
        <h2 className="mb-2 px-3 text-xs text-muted-foreground">Loading...</h2>
      )}
